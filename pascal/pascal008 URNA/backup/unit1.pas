...
begin // Carregar candidatos e suas imagens
  SetLength(FCandidatos, 5); // Exemplo com 3 candidatos
  ...
  FCandidatos[4].Numero := 24;
  FCandidatos[4].Nome := 'Putin';
  FCandidatos[4].Imagem := 'candidatos\putin.jpg';
end;
...
begin Encontrado := False;
  PlaySound(PChar('som\urna.mp3'), 0, SND_ASYNC);
  ...
  if Encontrado then
  begin
    PlaySound(PChar('som\urna.wav'), 0, SND_ASYNC);
  end
  else
  begin
...
begin
  Label4.Caption := 'Nome do Eleitor';
  Label3.Caption := '0';
  Image1.Picture := nil;
end;
